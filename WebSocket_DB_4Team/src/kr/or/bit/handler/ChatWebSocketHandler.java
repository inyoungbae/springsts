...
public class ChatWebSocketHandler extends TextWebSocketHandler {
	...
	private void createChatRoom(WebSocketSession session, JSONObject data) throws Exception {
		...
		ChatRoom room = new ChatRoom(data.getString("name"), Integer.parseInt(data.getString("max")));
	...
	private void sendChatRoomInfoMessage() throws Exception {
		...
		for (ChatRoom room : roomInfos.values()) {
			array.put(new JSONObject().put("max", room.getMax()).put("name", room.getName()).put("users", new JSONArray(room.getUserName())));
		}
