...
2. query : [레코드 여러개]의 값[객체목록]을 가져올때 사용 (게시판)    -- 게시판 전체목록 보기 
   --리턴타입  List<T>
...
@Component 그 외에 자동으로 스캔해서 등록하고 싶은 것들을 위해 사용            - Controller Service Repository 를 모두 포함하는 아이 
...
@Repository  //bean객체로 만들고싶다면? @Repository쓰면됨 + xml에다가 <Component scan을 쓰면 됨> 그럼 bean객체로 생성됨 
