...
//Inner Class 사용해서 구현 (Inner Class : 클래스안에 클래스를 쓰는 작업. 장점/이너클래스는 아우터 클래스의 자원을 사용할수있다)
//쓰레드를 2개만든건 동일한데, 서버와다르게 왜 INNER CLASS로 만들었나요?
//장점이뭐죠?
//쓰레드 클래스에서 socket을 그냥 갖다 쓸수있음. 
//아웃터클래스의 자원을 쓰기때문에, 별도객체 파라미터로 쓸필요없이. 그냥 갖다씀
//socket = new Socket("192.168.6.27",9999);이거를 그냥
//이너클래스가 갖다쓰고있음.
...
public class Ex04_TCP_Chatt_Client {
	...
	Ex04_TCP_Chatt_Client(){
		try {
			socket = new Socket("192.168.6.7",9999);
